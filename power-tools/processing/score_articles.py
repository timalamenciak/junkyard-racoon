...
def score_articles_llm(articles: list[dict], lab_profile: dict) -> None:
    ...
    for batch_start in range(0, len(articles), BATCH_SIZE):
        ...
        if not isinstance(scored, list):
            print(
                f"Warning: LLM scoring returned non-list payload for articles {batch_start}-{batch_start + len(batch) - 1}",
                file=sys.stderr,
            )
            continue
        for item in scored:
            if not isinstance(item, dict):
                print(
                    f"Warning: skipping malformed article score item in batch {batch_start}-{batch_start + len(batch) - 1}: {item!r}",
                    file=sys.stderr,
                )
                continue
